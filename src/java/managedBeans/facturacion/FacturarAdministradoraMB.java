...
public class FacturarAdministradoraMB extends MetodosGenerales implements Serializable {
    ...
    private CfgSede sedeActual;
    private CfgSede sedeTodas;
...
//        }
    ...
    public void confirmarGuardarFactura() {
        String numDocStr;
        int numDocInt = facturaAdmiFacade.consecutivoAdministradora(administradoraActual.getIdAdministradora())+1;
...
//        }
        ...
        nuevaFactura.setCodigoDocumento("FDA"+cerosIzquierda(numDocInt, 5));
        ...
        nuevaFactura.setResolucionDian("");
        ...
        RequestContext.getCurrentInstance().update("IdFormFacturacion");
        imprimirMensaje("Correcto", nuevaFactura.getTipoDocumento().getDescripcion() + " " + "FDA"+cerosIzquierda(numDocInt, 5) + " se ha creado.", FacesMessage.SEVERITY_INFO);
    }
    ...
    private void buscarItems() {
        ...
        if (!validarNoVacio(centroDeAtencion)) {
            return;
        }
        ...
        if (centroDeAtencion.compareTo("0") != 0) {
            sql = sql + "id_caja = (select id_caja from fac_caja where id_caja = fac_factura_paciente.id_caja and id_sede = " + centroDeAtencion + ") AND ";
        }
        sql = sql + "facturada_en_admi = false AND anulada = false and "
    ...
    public void cambiaSede() {
        if (validarNoVacio(centroDeAtencion)) {
            if (centroDeAtencion.compareTo("0") == 0) {
                sedeActual = sedeTodas;
            } else {
                sedeActual = sedesFacade.find(Integer.parseInt(centroDeAtencion));
            }
        } else {
            sedeActual = null;
        }
...
//        nuevaFactura.setFechaVencimiento("<b>FECHA VENCIMIENTO: </b>" + "-");
        ...
        //nuevaFactura.setNoGravadas("NO GRAVADA");
        nuevaFactura.setObservaciones(facturaAdmiSeleccionada.getObservacion());
        nuevaFactura.setSubtotal(format.format(facturaAdmiSeleccionada.getValorParcial()));
        //nuevaFactura.setIva(format.format(facturaAdmiSeleccionada.getIva() * facturaAdmiSeleccionada.getValorParcial() / 100));
        nuevaFactura.setTituloIva("IVA: ");
        nuevaFactura.setIva(format.format(facturaAdmiSeleccionada.getValoresIva()) + "");
        nuevaFactura.setTituloCree("CREE: ");
        nuevaFactura.setCree(format.format(facturaAdmiSeleccionada.getValoresCree()));
        //nuevaFactura.setCree(formateadorDecimal.format(facturaAdmiSeleccionada.getCree() * facturaAdmiSeleccionada.getValorParcial() / 100));

        nuevaFactura.setMenosCuotaModeradora(format.format(facturaAdmiSeleccionada.getValoresCuotaModeradora()));
        nuevaFactura.setMenosCopago(format.format(facturaAdmiSeleccionada.getValoresCopago()));
        String t = formateadorDecimal.format(facturaAdmiSeleccionada.getValorTotal());
        t = t.replace(".", ",");
        if (t.contains(",")) {
            t = t.split(",")[0];
        }
        nuevaFactura.setTotal(format.format(facturaAdmiSeleccionada.getValorTotal()));
        nuevaFactura.setSon("<b>SON: </b>" + ConversorDeNumerosALetras.convertNumberToLetter(t));
        nuevaFactura.setFirmaAutoriza(loginMB.getRutaCarpetaImagenes() + loginMB.getEmpresaActual().getLogo().getUrlImagen());
        nuevaFactura.setLogoEmpresa(loginMB.getRutaCarpetaImagenes() + loginMB.getEmpresaActual().getLogo().getUrlImagen());
        ...
        for (FacFacturaPaciente facturaPaciente : facturaAdmiSeleccionada.getFacFacturaPacienteList()) {
            ...
            for (FacFacturaMedicamento medicamentoFactura : facturaPaciente.getFacFacturaMedicamentoList()) {
                ...
                nuevoItem.setDescripcion(medicamentoFactura.getIdMedicamento().getNombreMedicamento());
                nuevoItem.setValorUnitario(format.format(medicamentoFactura.getValorMedicamento()));
                nuevoItem.setValorTotal(format.format(medicamentoFactura.getValorParcial()));
                listaItemsFactura.add(nuevoItem);
            }
            for (FacFacturaInsumo insumoFactura : facturaPaciente.getFacFacturaInsumoList()) {
                EstructuraItemsAdministradora nuevoItem = new EstructuraItemsAdministradora();
                ...
                nuevoItem.setDescripcion(insumoFactura.getIdInsumo().getNombreInsumo());
                nuevoItem.setValorUnitario(format.format(insumoFactura.getValorInsumo()));
                nuevoItem.setValorTotal(format.format(insumoFactura.getValorParcial()));
                listaItemsFactura.add(nuevoItem);
            }
            for (FacFacturaPaquete paqueteFactura : facturaPaciente.getFacFacturaPaqueteList()) {
                EstructuraItemsAdministradora nuevoItem = new EstructuraItemsAdministradora();
                ...
                for(FacPaqueteServicio servicio: paqueteFactura.getIdPaquete().getFacPaqueteServicioList()){
                    nuevoItem = new EstructuraItemsAdministradora();
    ...
    public void confirmarAnularFactura() {
        ...
        for(FacFacturaPaciente fac : facturaAdmiSeleccionada.getFacFacturaPacienteList()){
            fac.setFacturadaEnAdmi(false);
            facturaPacienteFacade.edit(fac);
        }
    ...
    public FacFacturaAdmiFacade getFacturaAdmiFacade() {
        return facturaAdmiFacade;
    }

    public void setFacturaAdmiFacade(FacFacturaAdmiFacade facturaAdmiFacade) {
        this.facturaAdmiFacade = facturaAdmiFacade;
    }
    ...
    public FacFacturaAdmi getFacturaAdmiSeleccionada() {
        return facturaAdmiSeleccionada;
    }

    public void setFacturaAdmiSeleccionada(FacFacturaAdmi facturaAdmiSeleccionada) {
        this.facturaAdmiSeleccionada = facturaAdmiSeleccionada;
    }

    public FilaDataTable getFacturaAdmiSeleccionadaTabla() {
        return facturaAdmiSeleccionadaTabla;
    }

    public void setFacturaAdmiSeleccionadaTabla(FilaDataTable facturaAdmiSeleccionadaTabla) {
        this.facturaAdmiSeleccionadaTabla = facturaAdmiSeleccionadaTabla;
    }
    ...
    public FacContrato getContratoTodos() {
        return contratoTodos;
    }

    public void setContratoTodos(FacContrato contratoTodos) {
        this.contratoTodos = contratoTodos;
    }
    ...
    public List<FacFacturaPaciente> getListaFacturasDePacientes() {
        return listaFacturasDePacientes;
    }

    public void setListaFacturasDePacientes(List<FacFacturaPaciente> listaFacturasDePacientes) {
        this.listaFacturasDePacientes = listaFacturasDePacientes;
    }
    ...
    public List<EstructuraFacturaAdministradora> getListaRegistrosParaPdf() {
        return listaRegistrosParaPdf;
    }

    public void setListaRegistrosParaPdf(List<EstructuraFacturaAdministradora> listaRegistrosParaPdf) {
        this.listaRegistrosParaPdf = listaRegistrosParaPdf;
    }

    public int getFacturasSinAutorizacionCerradaInt() {
        return facturasSinAutorizacionCerradaInt;
    }

    public void setFacturasSinAutorizacionCerradaInt(int facturasSinAutorizacionCerradaInt) {
        this.facturasSinAutorizacionCerradaInt = facturasSinAutorizacionCerradaInt;
    }

    public int getFacturasFueraDeRangoInt() {
        return facturasFueraDeRangoInt;
    }

    public void setFacturasFueraDeRangoInt(int facturasFueraDeRangoInt) {
        this.facturasFueraDeRangoInt = facturasFueraDeRangoInt;
    }

    public CfgSedeFacade getSedesFacade() {
        return sedesFacade;
    }

    public void setSedesFacade(CfgSedeFacade sedesFacade) {
        this.sedesFacade = sedesFacade;
    }

    public CfgSede getSedeActual() {
        return sedeActual;
    }

    public void setSedeActual(CfgSede sedeActual) {
        this.sedeActual = sedeActual;
    }

    public CfgSede getSedeTodas() {
        return sedeTodas;
    }

    public void setSedeTodas(CfgSede sedeTodas) {
        this.sedeTodas = sedeTodas;
    }
