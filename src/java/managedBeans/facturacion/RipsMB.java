...
public class RipsMB extends MetodosGenerales implements Serializable {
    ...
    private final DecimalFormat formateadorDecimal = new DecimalFormat("0");
    ...
    private ArrayList<String> determinarDiagnosticos(FacFacturaServicio facturaServicio) {
        FacFacturaPaciente facturaPaciente = facturaServicio.getFacFacturaPaciente();
        ...
        if (registroConDiagnostico == null) {//se busca el ultimo registro del paciente
            System.out.println("sin cita");
            registroConDiagnostico = registroFacade.buscarRegistroConDiagnosticoSegunPaciente(facturaServicio);
            System.out.println("id paciente "+facturaServicio.getFacFacturaPaciente().getIdPaciente().getIdPaciente());
        }
        if (registroConDiagnostico == null) {//no se encontro un registro con diagnostico
            return null;
        } 
        System.out.println("Tipo: "+registroConDiagnostico.getIdTipoReg().getIdTipoReg());
        System.out.println("Registro: "+registroConDiagnostico.getIdRegistro());
        ...
        switch (registroConDiagnostico.getIdTipoReg().getIdTipoReg()) {
            ...
            case 8://8;"HISTORIA CLINICA ADULTO"(4 dignosticos)
                ...
                break;
            case 54://1;"HISTORIA CLINICA RCV"   (5 diagnosticos)
                for (HcDetalle detalle : registroConDiagnostico.getHcDetalleList()) {
                    try {
                        switch (detalle.getHcCamposReg().getPosicion()) {
                            case 166://dx principal
                                arregloRetorno.set(0, detalle.getValor().split(" - ")[0]);
                                break;
                            case 167://dx rel_2
                                arregloRetorno.set(1, detalle.getValor().split(" - ")[0]);
                            ...
                            case 169://dx rel_4
                                ...
                                break;
                            case 268://dx rel_5
                                arregloRetorno.set(4, detalle.getValor().split(" - ")[0]);
                                break;
                            case 335://tipo
                                System.out.println("TI: " + clasificacionesFacade.buscarPorId(detalle.getValor()).getCodigo());
                                arregloRetorno.set(5, clasificacionesFacade.buscarPorId(detalle.getValor()).getCodigo());
                                break;
                            case 336://causa ext
                                System.out.println("CE: " + clasificacionesFacade.buscarPorId(detalle.getValor()).getCodigo());
                                arregloRetorno.set(6, clasificacionesFacade.buscarPorId(detalle.getValor()).getCodigo());
                                break;
                            case 337://finalidad
                                System.out.println("FI: " + clasificacionesFacade.buscarPorId(detalle.getValor()).getCodigo());
                                arregloRetorno.set(7, clasificacionesFacade.buscarPorId(detalle.getValor()).getCodigo());
                                break;
                        }
                    } catch (Exception e) {
                        System.out.println("Error al sacar codigo de diagnostico de: " + detalle.getValor());
                ...
                break;
            case 77://1;"HISTORIA CLINICA DE NUTRICION"   (5 diagnosticos)
                for (HcDetalle detalle : registroConDiagnostico.getHcDetalleList()) {
                    try {
                        switch (detalle.getHcCamposReg().getPosicion()) {
                            case 191://dx principal
                                arregloRetorno.set(0, detalle.getValor().split(" - ")[0]);
                                break;
                            case 192://dx rel_2
                                arregloRetorno.set(1, detalle.getValor().split(" - ")[0]);
            ...
            for (FacFacturaAdmi facturaActual : listaFacturasAdministradora) {
                if (ripAF) {//RIP facturas_y_fac_extramurales
                    for (FacFacturaPaciente facturaPaciente : facturaActual.getFacFacturaPacienteList()) {
                        ...
                        nuevoregistroRipAF.setNumFac(facturaPaciente.getNumeroDocumento().toString());
                        nuevoregistroRipAF.setFecExp(formatoFechaSql.format(facturaPaciente.getFechaElaboracion()));//cuando se creo la factura
                        ...
                        nuevoregistroRipAF.setNumCon(facturaPaciente.getIdContrato().getCodigoContrato());
                        nuevoregistroRipAF.setPlanBen("POS");//Plan de Beneficios: NO SE MANEJA VA VACIO
                        nuevoregistroRipAF.setNumPoli(facturaPaciente.getIdContrato().getNumeroPoliza());//Número de la póliza(SALE DE TABLA CONTRATO)
                        ...
                        nuevoregistroRipAF.setValNet(facturaPaciente.getValorEmpresa());// Valor Neto a Pagar por la entidad Contratante: valor paga
                    ...
                    for (FacFacturaPaciente facturaPaciente : facturaActual.getFacFacturaPacienteList()) {
                        ...
                        if (ripUsBuscado == null) {//no se encuentra en el archivo de usuarios por lo que debe ser agregado
                            ...
                            if (facturaPaciente.getIdPaciente().getMunicipio() != null) {
                                nuevoregistroRipUS.setCodMunRes(facturaPaciente.getIdPaciente().getMunicipio().getCodigo());//SE IMPRIME EL CODIGO DEL MUNICIPIO SEGUN NORMA - CORREGIDO POR JEANFRANCO CORES                        
                            } else {
                ...
                if (ripAP) {
                    ...
                    for (FacFacturaPaciente facturaPaciente : facturaActual.getFacFacturaPacienteList()) {
                        for (FacFacturaServicio servicioActual : facturaPaciente.getFacFacturaServicioList()) {                            
                            ...
                            if(servicioActual.getIdServicio().getCodigoCup().startsWith("8")){
                                if(!servicioActual.getIdServicio().getCodigoCup().equals("890114")){
                                    continuar=false;
                                }
                            }
                            ...
                            if (continuar) {
                                ...
                                nuevoregistroRipAP.setNumFac(facturaPaciente.getNumeroDocumento().toString());
...
//                                }
                ...
                if (ripAC) {
                    ...
                    for (FacFacturaPaciente facturaPaciente : facturaActual.getFacFacturaPacienteList()) {
                        ...
                        for (FacFacturaServicio servicioActual : facturaPaciente.getFacFacturaServicioList()) {
                            ...
                            if (continuar && servicioActual.getIdServicio().getCodigoCup().equals("890114")) {
                                continuar = false;
                            }
                            if (continuar) {
                                ArrayList<String> diagnosticosDeterminados = determinarDiagnosticos(servicioActual);
    ...
    public StreamedContent generarZipfacturaAdministradora() throws JRException, IOException {
        ...
        if (facturarAdministradoraMB.getFacturaSeleccionadaTabla() != null) {
            ...
            String nombrerips = "000000",rip = (Integer.parseInt(ripsAlmacenadosFacade.contRips().toString())+1)+"";
            nombreRIPS = nombrerips.substring(0, nombrerips.length()-rip.length())+rip;
...
//                        pw.println((diagnostico.getCodigoDiagnostico() + "," + diagnostico.getNombreDiagnostico()).trim());
                    }
                } catch (Exception e) {
            ...
            if (elem.toString().trim().compareTo("CT") == 0) {
                ...
                try {
                    fichero = new FileWriter(nameAndPathFile);
                    PrintStream pw = new PrintStream(ficherofile);
                    int i = 0;
                    for (RipsCt ct : ripSeleccionado.getRipsCtList()) { 
                        ...
                        System.out.println(i+" "+ripSeleccionado.getRipsCtList().size());
                        if(i < ripSeleccionado.getRipsCtList().size()-1){
                            pw.print(ln);
                        }
                        i++;
                    }
...
//                        pw.println("cod_pre,raz_soc,tip_ide,num_ide,num_fac,fec_exp,fec_inc,fec_fin,cod_ent,nom_ent,num_con,plan_ben,num_poli,val_copa,val_com,val_desc,val_net");
                    ...
                    for (RipsAf af : ripSeleccionado.getRipsAfList()) {
                        ...
                        if(i < ripSeleccionado.getRipsAfList().size()-1){
                            pw.print(ln);
                        }
                        i++;
                    }
...
//                        pw.println("num_fac,cod_pre,tip_ide,num_ide,fec_proc,num_aut,cod_proced,ambito_pro,finali_pro,pers_atiend,diag_pri,diag_relac,complica,act_quirur,val_proced");
                    ...
                    for (RipsAp ap : ripSeleccionado.getRipsApList()) {
                        ...
                        if(i < ripSeleccionado.getRipsApList().size()-1){
                            pw.print(ln);
                        }
                        i++;
                    }
