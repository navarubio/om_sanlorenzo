...
public class ManualesTarifariosMB extends MetodosGenerales implements Serializable {
   ...
   // private String idServicio = "";
    // Lista de selección multiple de servicios
    private List<String> lstServicioSeleccionado;
    ...
    public void recargarFilasTablas() {
        ...
        if (manualSeleccionado != null) {
            ...
            for (FacManualTarifarioServicio servicioManual : listaServiciosManualTarifario) {//lista servicios
                ...
                nuevaFila.setColumna6(String.valueOf(Math.round(servicioManual.getValorInicial())));
                ...
                nuevaFila.setColumna9(String.valueOf(Math.round(servicioManual.getValorFinal())));
    ...
    public void changeServicio(ValueChangeEvent evt){
        String values = evt.getNewValue().toString().replace("[", "");
        values = values.replace("]", "");
        String valores[] = values.split(",");
        servicioSeleccionado = valores[valores.length-1];
    }
    public void calcularValoresServicio() {
        listaServicios.clear();
        listaServicios = servicioFacade.buscarNoEstanEnManualXTarifa(manualSeleccionado.getIdManualTarifario(),tipoTarifa);
        unidadValor = "";
        listaUnidadesValor = unidadValorFacade.buscarOrdenado();
        if (listaUnidadesValor != null && !listaUnidadesValor.isEmpty()) {
            int anioActual = (new Date()).getYear() + 1900;
        ...
        //if (!lstServicioSeleccionado.isEmpty()) {
           //for(int i=0 ; i<lstServicioSeleccionado.size();i++){
           if(!servicioSeleccionado.equals("")){
            FacServicio s = servicioFacade.find(Integer.parseInt(servicioSeleccionado.trim()));
            if (s != null) {
                switch (tipoTarifa) {
                    case "Especifica":
                    ...
                    case "SOAT":
                        if (validarNoVacio(unidadValor)) {
                            ...
                            valorInicialServicio = s.getFactorSoat()* f.getSmlvd();
                            valorFinalServicio = valorInicialServicio - (valorInicialServicio * (descuentoServicio / 100)) + honorarioMedico;
                        } else {
                    ...
                    case "ISS":
                        if (validarNoVacio(unidadValor)) {
                            FacUnidadValor f = unidadValorFacade.find(Integer.parseInt(unidadValor));
                            valorInicialServicio = s.getFactorIss()* f.getUvr();
                            valorFinalServicio = valorInicialServicio - (valorInicialServicio * (descuentoServicio / 100)) + honorarioMedico;
                        } else {
                            imprimirMensaje("Alerta", "No hay una unidad de valor para calcular el valor final", FacesMessage.SEVERITY_WARN);
                        }
...
//
    public void cargarDialogoAgregarServicio() {
        ...
        lstServicioSeleccionado = new ArrayList();
        listaServicios = servicioFacade.buscarNoEstanEnManualXTarifa(manualSeleccionado.getIdManualTarifario(),tipoTarifa);
    ...
    public void agregarServicio() {
        if (lstServicioSeleccionado.isEmpty()) {
            imprimirMensaje("Error", "No ha seleccionado ningun servicio.", FacesMessage.SEVERITY_ERROR);
            return;
        ...
        for (int i = 0; i < lstServicioSeleccionado.size(); i++) {
            FacServicio s = servicioFacade.find(Integer.parseInt(lstServicioSeleccionado.get(i)));
            switch(tipoTarifa){
                case "Especifica":
                    valorInicialServicio = s.getValorParticular();
                    valorFinalServicio = valorInicialServicio - (valorInicialServicio * (descuentoServicio / 100)) + honorarioMedico;
            ...
            llavePK.setIdServicio(Integer.parseInt(lstServicioSeleccionado.get(i)));
            FacManualTarifarioServicio buscado = manualTarifarioServicioFacade.find(llavePK);
            if (buscado != null) {//validar que no exista
                imprimirMensaje("Error", "Este manual tarifario ya tiene adicionado algunos de los servicios seleccionados.", FacesMessage.SEVERITY_ERROR);
                return;
...
//    
    ...
    public List<String> getLstServicioSeleccionado() {
        return lstServicioSeleccionado;
    }

    public void setLstServicioSeleccionado(List<String> lstServicioSeleccionado) {
        this.lstServicioSeleccionado = lstServicioSeleccionado;
    }
