...
public class FacturarPacienteMB extends MetodosGenerales implements Serializable {
    ...
    private String medico;//Medicado asociado a la historia clinica por medio de una cita
    private double valorEmpresa = 0;
    private double valorEmpresaServicio = 0;
    private double valorUsuario = 0;
    private double valorUsuarioServicio = 0;
    private double valorParcial = 0;
    private double valorTotal = 0;

    private double valorCopago = 0;
    private double valorCopagoTotal = 0;
    private String msjHtmlCopago = "";

    private double valorCuotaModeradora = 0;
    private double valorCuotaModeradoraTotal = 0;
    ...
    public void cargarDesdeTab(String id) {//cargar paciente y servicios desde tab recepcionCitas
        ...
        if (splitId[0].compareTo("idCita") == 0) {
            ...
            if (pacienteTmp == null) {
                imprimirMensaje("Error", "No se encontrÃ³ el paciente correspondiente a la cita", FacesMessage.SEVERITY_ERROR);
            ...
            pacienteSeleccionado = pacienteTmp;
            idContratoActual = pacienteSeleccionadoTabla.getIdContrato().getIdContrato().toString();
            ...
            if (servicioEnManualTarifario == null) {//servicio no se encuentra en el manual tarifario                
                ...
                idServicioManual = String.valueOf(citaActual.getIdServicio().getIdServicio());
                cantidadServicio = 1;
    ...
    public void cambiaContrato() {//funcion cuando se selecciona otro contrato del listado
        listaServiciosFactura.clear();
        listaServiciosFacturaFiltro.clear();
        listaMedicamentosFactura.clear();
        listaMedicamentosFacturaFiltro.clear();
        listaInsumosFactura.clear();
    ...
    private void determinarImpuestos() {
        ...
        //IVA        
        msjHtmlIva = "";
        if (contratoActual.getAplicarIva()) {//buscar si en la tabla impuestos hay un valor para el aÃ±o de la fecha
        ...
        //CREE
        msjHtmlCree = "";
        if (contratoActual.getAplicarCree()) {//buscar si en la tabla impuestos hay un valor para el aÃ±o de la fecha
    ...
    private boolean validarManualTarifario() {
        ...
        if (pacienteSeleccionado.getRegimen() == null || pacienteSeleccionado.getIdAdministradora() == null) {
            mensajeConfiguracion = "No se puede facturar: \n"
                    + "Paciente: " + pacienteSeleccionado.nombreCompleto() + " \n"
                    + "RazÃ³n: El paciente no tiene rÃ©gimen Ã³ administradora";
            return false;
        }
        ...
        if (pacienteSeleccionado.getIdAdministradora().getIdAdministradora() != 1) {//ADMINISTRADORA PACIENTE NO ES PARTICULAR, SE DEBEN REALIZAR LAS CORESPONDIENTES VALIDACIONES
            ...
            if (administradoraParticular == null) {//validar que exista la administradora particular y tenga manual tarifario
                mensajeConfiguracion = "No se puede facturar: \n"
                        + "Paciente: " + pacienteSeleccionado.nombreCompleto() + " \n"
                        + "RazÃ³n: Es obligatorio que exista una administradora PARTICULAR";
                return false;
            }
            ...
            if (administradoraParticular.getRazonSocial().compareTo("PARTICULAR") != 0) {//la primer administradora debe ser PARTICULAR (no puede ser otra)
                mensajeConfiguracion = "No se puede facturar: \n"
                        + "Paciente: " + pacienteSeleccionado.nombreCompleto() + " \n"
                        + "RazÃ³n: La primer administradora ( id = 1 ) debe llamarse PARTICULAR obligatoriamente";
                return false;
            }
            ...
            if (administradoraParticular.getFacContratoList() == null || administradoraParticular.getFacContratoList().isEmpty()) {//debe tener un contrato
                mensajeConfiguracion = "No se puede facturar: \n"
                        ...
                        + "RazÃ³n: La administradora no tiene contratos";
                return false;
            }
            ...
            for (FacContrato contrato : administradoraParticular.getFacContratoList()) {
                if (contrato.getIdManualTarifario() == null) {//debe contar con el manual tarifario                    
                    mensajeConfiguracion = "No se puede facturar: \n"
                            ...
                            + "RazÃ³n: El contrato no tiene manual tarifario";
                    return false;
                }
        ...
        if (listaContratosAplican.isEmpty()) {
            mensajeConfiguracion = "No se puede facturar: \n"
                    + "Paciente: " + pacienteSeleccionado.nombreCompleto() + " \n"
                    + "Administradora: " + pacienteSeleccionado.getIdAdministradora().getRazonSocial() + " \n"
                    + "RazÃ³n: La administradora no tiene contratos de tipo " + pacienteSeleccionado.getRegimen().getDescripcion() + "\n"
                    + "Ã³ de tenerlo no tiene asociado un manual tarifario.";
            return false;
        }
    ...
    public void validarTurno() {
        ...
        if (turnoActual != null) {
            msjHtmlTurnoCita = "Correcto se <br/>asociarÃ¡ este turno";
            estiloTurnoCita = "";
        } else {
            msjHtmlTurnoCita = "Se puede facturar <br/>pero tener en cuenta <br/>que al no asociar un turno <br/>la generacion de RIPS <br/>podrÃ­a salir incompleta.";
...
//        }
    ...
    public void generarPdf_Factura(FacFacturaPaciente fac) throws JRException, IOException {//genera un pdf de una historia seleccionada en el historial        
        facturaSeleccionada = facturaPacienteFacade.find(fac.getIdFacturaPaciente());
        tipoDocumentoFacturaSeleccionada = facturaSeleccionada.getTipoDocumento().getDescripcion();//determinar si se imprime media o pagina completa        
        JRBeanCollectionDataSource beanCollectionDataSource;
        switch (tipoDocumentoFacturaSeleccionada) {
            case "Recibo de Caja"://se imprime a media hoja
    ...
    private boolean cargarFuenteDeDatosFactura() {
        ...
        nuevaFactura.setIva(format.format(facturaSeleccionada.getIva() * facturaSeleccionada.getValorParcial() / 100));
        nuevaFactura.setTituloCree("CREE: (" + format.format(facturaSeleccionada.getCree()) + "%)");
        nuevaFactura.setTituloIva("IVA: (" + format.format(facturaSeleccionada.getIva()) + "%)");
        nuevaFactura.setCree(format.format(facturaSeleccionada.getCree() * facturaSeleccionada.getValorParcial() / 100));

        nuevaFactura.setMenosCuotaModeradora(format.format(facturaSeleccionada.getCuotaModeradora()));
        nuevaFactura.setMenosCopago(format.format(facturaSeleccionada.getCopago()));
        ...
        for (FacFacturaServicio servicioFactura : facturaSeleccionada.getFacFacturaServicioList()) {
            ...
            nuevoItem.setValorUnitario(format.format(servicioFactura.getValorServicio()));
            nuevoItem.setValorTotal(format.format(servicioFactura.getValorParcial()));
    ...
    private boolean cargarFuenteDeDatosReciboCaja() {
        ...
        nuevoReciboCaja.setClienteIdentificacion("<b>IDENTIFICACIÓN: </b>" + facturaSeleccionada.getIdPaciente().getIdentificacion());
    ...
    public void confirmarAnularFactura() {
        if (facturaSeleccionadaTabla == null) {
            imprimirMensaje("Error", "No se ha seleccionado ningÃºn factura", FacesMessage.SEVERITY_ERROR);
            return;
        }
        facturaSeleccionada = facturaPacienteFacade.find(Integer.parseInt(facturaSeleccionadaTabla.getColumna1()));
    ...
    public void confirmarGuardarFactura(ActionEvent actionEvent) {
        try {
            ...
            if (Objects.equals(consecutivoSeleccionado.getActual(), consecutivoSeleccionado.getFin())) {//se vuelve a comprobar el consecutivo
                imprimirMensaje("Error", "Otro usuario utilizÃ³ el Ãºltimo consecutivo para el tipo de documento seleccionado", FacesMessage.SEVERITY_ERROR);
                cambiaDocumento();
                RequestContext.getCurrentInstance().update("IdFormFacturacion");
                return;
            } else {
            ...
            for (FilaDataTable paqueteFactura : listaPaquetesFactura) {
                try {
                    ...
                    nuevoPaqueteFactura.setValorPaquete(Double.parseDouble(paqueteFactura.getColumna5().replaceAll(",", ".")));//nuevaFila.setColumna5(String.valueOf(valorUnitarioPaquete));
                    ...
                    nuevoPaqueteFactura.setValorParcial(Double.parseDouble(paqueteFactura.getColumna7().replaceAll(",", ".")));//nuevaFila.setColumna7(String.valueOf(valorFinalPaquete));
    ...
    private void recalcularValorFactura() {
        double vlr = 0;
        for (FilaDataTable servicioFactura : listaServiciosFactura) {
            vlr = vlr + Double.parseDouble(servicioFactura.getColumna7().replaceAll(",", "."));
        }
        for (FilaDataTable medicamentoFactura : listaMedicamentosFactura) {
            vlr = vlr + Double.parseDouble(medicamentoFactura.getColumna7());
        }
        for (FilaDataTable paqueteFactura : listaPaquetesFactura) {
            vlr = vlr + Double.parseDouble(paqueteFactura.getColumna7().replaceAll(",", "."));
        }
        for (FilaDataTable insumoFactura : listaInsumosFactura) {
...
//        idPrestadorServicio = "";
    ...
    public void calcularValoresServicio() {//calculo del valor final de un servicio
        if (validarNoVacio(idServicioManual)) {
            ...
            if (!facturarComoParticular) {
                ...
                valorEmpresaServicio = valor - valorUsuarioServicio + (valorUnitarioServicio * (valorIva / 100)) + (valorUnitarioServicio * (valorCree / 100));
            }
        }
    }
    ...
    public void quitarServicioFactura() {
        if (servicioFacturaSeleccionado == null) {
            imprimirMensaje("Error", "No se ha seleccionado ningÃºn servicio", FacesMessage.SEVERITY_ERROR);
            return;
        }
...
//        idPrestadorMedicamento = "";
    ...
    public void quitarMedicamentoFactura() {
        if (medicamentoFacturaSeleccionado == null) {
            imprimirMensaje("Error", "No se ha seleccionado ningÃºn medicamento", FacesMessage.SEVERITY_ERROR);
            return;
        }
...
//        idPrestadorInsumo = "";
    ...
    public void agregarInsumosDesdeConsumos() {//agregar desde el dialogo de consumos
        if (insumosConsumoSeleccionados == null || insumosConsumoSeleccionados.isEmpty()) {
            imprimirMensaje("Error", "Ne se ha seleccionado ningÃºn insumo", FacesMessage.SEVERITY_ERROR);
        } else {
            FilaDataTable nuevaFila;
    ...
    public void quitarInsumoFactura() {
        if (insumoFacturaSeleccionado == null) {
            imprimirMensaje("Error", "No se ha seleccionado ningÃºn insumo", FacesMessage.SEVERITY_ERROR);
            return;
        }
        try {
...
//        idPrestadorPaquete = "";
    ...
    public void agregarPaqueteAFactura() {//agregar desde la seccion de items de factura
        ...
        nuevaFila.setColumna5(formateadorDecimal.format(valorUnitarioPaquete));
        nuevaFila.setColumna6(String.valueOf(cantidadPaquete));
        nuevaFila.setColumna7(formateadorDecimal.format(valorFinalPaquete));
        nuevaFila.setColumna8(usuariosFacade.find(Integer.parseInt(idPrestadorPaquete)).nombreCompleto());
        nuevaFila.setColumna20(pacienteSeleccionado.getIdPaciente().toString());
    ...
    public void agregarPaquetesDesdeConsumos() {//agregar desde el dialogo de consumos
        if (paquetesConsumoSeleccionados == null || paquetesConsumoSeleccionados.isEmpty()) {
            imprimirMensaje("Error", "Ne se ha seleccionado ningÃºn paquete", FacesMessage.SEVERITY_ERROR);
        } else {
            FilaDataTable nuevaFila;
    ...
    public void quitarPaqueteFactura() {
        if (paqueteFacturaSeleccionado == null) {
            imprimirMensaje("Error", "No se ha seleccionado ningÃºn paquete", FacesMessage.SEVERITY_ERROR);
            return;
        }
        try {
    ...
    public FacFacturaPacienteFacade getFacturaPacienteFacade() {
        return facturaPacienteFacade;
    }

    public void setFacturaPacienteFacade(FacFacturaPacienteFacade facturaPacienteFacade) {
        this.facturaPacienteFacade = facturaPacienteFacade;
    }
    ...
    public FacCajaFacade getCajaFacade() {
        return cajaFacade;
    }

    public void setCajaFacade(FacCajaFacade cajaFacade) {
        this.cajaFacade = cajaFacade;
    }

    public FacPeriodoFacade getPeriodoFacade() {
        return periodoFacade;
    }

    public void setPeriodoFacade(FacPeriodoFacade periodoFacade) {
    ...
    public FacImpuestosFacade getImpuestosFacade() {
        return impuestosFacade;
    }

    public void setImpuestosFacade(FacImpuestosFacade impuestosFacade) {
        this.impuestosFacade = impuestosFacade;
    }
    ...
    public FacConsecutivoFacade getConsecutivoFacade() {
        return consecutivoFacade;
    }

    public void setConsecutivoFacade(FacConsecutivoFacade consecutivoFacade) {
        this.consecutivoFacade = consecutivoFacade;
    }

    public FacManualTarifarioFacade getManualTarifarioFacade() {
        return manualTarifarioFacade;
    }

    public void setManualTarifarioFacade(FacManualTarifarioFacade manualTarifarioFacade) {
        this.manualTarifarioFacade = manualTarifarioFacade;
    }
    ...
    public FacFacturaServicioFacade getFacturaServicioFacade() {
        return facturaServicioFacade;
    }

    public void setFacturaServicioFacade(FacFacturaServicioFacade facturaServicioFacade) {
        this.facturaServicioFacade = facturaServicioFacade;
    }

    public FacFacturaInsumoFacade getFacturaInsumoFacade() {
        return facturaInsumoFacade;
    }

    public void setFacturaInsumoFacade(FacFacturaInsumoFacade facturaInsumoFacade) {
        this.facturaInsumoFacade = facturaInsumoFacade;
    }

    public FacFacturaMedicamentoFacade getFacturaMedicamentoFacade() {
        return facturaMedicamentoFacade;
    }

    public void setFacturaMedicamentoFacade(FacFacturaMedicamentoFacade facturaMedicamentoFacade) {
        this.facturaMedicamentoFacade = facturaMedicamentoFacade;
    }

    public FacFacturaPaqueteFacade getFacturaPaqueteFacade() {
        return facturaPaqueteFacade;
    }

    public void setFacturaPaqueteFacade(FacFacturaPaqueteFacade facturaPaqueteFacade) {
        this.facturaPaqueteFacade = facturaPaqueteFacade;
    }

    public FacUnidadValorFacade getUnidadValorFacade() {
        return unidadValorFacade;
    }

    public void setUnidadValorFacade(FacUnidadValorFacade unidadValorFacade) {
        this.unidadValorFacade = unidadValorFacade;
    }

    public FacConsumoServicioFacade getConsumoServicioFacade() {
        return consumoServicioFacade;
    }

    public void setConsumoServicioFacade(FacConsumoServicioFacade consumoServicioFacade) {
        this.consumoServicioFacade = consumoServicioFacade;
    }

    public FacConsumoMedicamentoFacade getConsumoMedicamentoFacade() {
        return consumoMedicamentoFacade;
    }

    public void setConsumoMedicamentoFacade(FacConsumoMedicamentoFacade consumoMedicamentoFacade) {
        this.consumoMedicamentoFacade = consumoMedicamentoFacade;
    }

    public FacConsumoInsumoFacade getConsumoInsumoFacade() {
        return consumoInsumoFacade;
    }

    public void setConsumoInsumoFacade(FacConsumoInsumoFacade consumoInsumoFacade) {
        this.consumoInsumoFacade = consumoInsumoFacade;
    }

    public FacConsumoPaqueteFacade getConsumoPaqueteFacade() {
        return consumoPaqueteFacade;
    }

    public void setConsumoPaqueteFacade(FacConsumoPaqueteFacade consumoPaqueteFacade) {
        this.consumoPaqueteFacade = consumoPaqueteFacade;
    }

    public FacContrato getContratoActual() {
        return contratoActual;
    }

    public void setContratoActual(FacContrato contratoActual) {
    ...
    public FacFacturaPaciente getFacturaSeleccionada() {
        return facturaSeleccionada;
    }

    public void setFacturaSeleccionada(FacFacturaPaciente facturaSeleccionada) {
    ...
    public String getTipoDocumentoFacturaSeleccionada() {
        return tipoDocumentoFacturaSeleccionada;
    }

    public void setTipoDocumentoFacturaSeleccionada(String tipoDocumentoFacturaSeleccionada) {
        this.tipoDocumentoFacturaSeleccionada = tipoDocumentoFacturaSeleccionada;
    }

    public List<EstructuraFacturaPaciente> getListaRegistrosFactura() {
        return listaRegistrosFactura;
    }

    public void setListaRegistrosFactura(List<EstructuraFacturaPaciente> listaRegistrosFactura) {
        this.listaRegistrosFactura = listaRegistrosFactura;
    }

    public List<EstructuraReciboCaja> getListaRegistrosReciboCaja() {
        return listaRegistrosReciboCaja;
    }

    public void setListaRegistrosReciboCaja(List<EstructuraReciboCaja> listaRegistrosReciboCaja) {
        this.listaRegistrosReciboCaja = listaRegistrosReciboCaja;
    }
    ...
    public CitTurnos getTurnoActual() {
        return turnoActual;
    }

    public void setTurnoActual(CitTurnos turnoActual) {
        this.turnoActual = turnoActual;
    }

    public double getValorEmpresaServicio() {
        return valorEmpresaServicio;
    }

    public void setValorEmpresaServicio(double valorEmpresaServicio) {
        this.valorEmpresaServicio = valorEmpresaServicio;
    }

    public double getValorUsuarioServicio() {
        return valorUsuarioServicio;
    }

    public void setValorUsuarioServicio(double valorUsuarioServicio) {
        this.valorUsuarioServicio = valorUsuarioServicio;
    }

    public double getValorCopagoTotal() {
        return valorCopagoTotal;
    }

    public void setValorCopagoTotal(double valorCopagoTotal) {
        this.valorCopagoTotal = valorCopagoTotal;
    }

    public double getValorCuotaModeradoraTotal() {
        return valorCuotaModeradoraTotal;
    }
