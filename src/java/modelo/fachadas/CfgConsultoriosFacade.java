...
public class CfgConsultoriosFacade extends AbstractFacade<CfgConsultorios> {
    ...
    public int numeroConsultorios(String codigoConsultorio) {
        try {
            
            Query q = getEntityManager().createQuery("SELECT COUNT(c) FROM CfgConsultorios  c WHERE c.codConsultorio = ?1");
            q.setParameter(1, codigoConsultorio);
