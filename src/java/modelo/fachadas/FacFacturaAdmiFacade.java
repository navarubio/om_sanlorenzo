...
public class FacFacturaAdmiFacade extends AbstractFacade<FacFacturaAdmi> {
    ...
    public int consecutivoAdministradora(int idAdministradora) {
        try {
            return Integer.parseInt(getEntityManager().createNativeQuery("SELECT COUNT(*) FROM fac_factura_admi WHERE id_administradora = "+idAdministradora).getSingleResult().toString());
